...
newStopWords = ["said", "one", "go", "went", "came", "like", "one", "line", "ital", "im", "oh", "hey", "thats"]
...
bojackTextPath = os.path.join(workingDir, 'bojackText')
print(bojackTextPath)
...
allDocs = []
for file in os.listdir(bojackTextPath):
    if file.endswith(".txt"):
        filepath = f"{bojackTextPath}/{file}"
        # print(filepath)
...
sorted_topics = sorted(topics[10], key=lambda x: x[1], reverse=True)
